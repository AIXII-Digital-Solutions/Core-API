"""cirium: split asg/delta into per-plan_type matviews (commercial / business_helicopters / full)

Revision ID: c0d1e2f3a4b5
Revises: b9c0d1e2f3a4
Create Date: 2026-07-01

Replaces the single cirium.asg / cirium.delta matviews (top-3 global revisions) with per-plan_type
variants whose "current" = the LATEST revision of that plan_type and "old" = the older revisions of
the SAME plan_type. All the rest of the asg / delta logic (airline match / is_active, changed-rows /
is_latest, column projection) is byte-for-byte the d9e0 definition — only the revision scoping
changed. asg_full / delta_full = UNION ALL of the two plan variants. api.sync_registration_from_asg()
is repointed from cirium.asg to cirium.asg_full.

GENERATED from d9e0 by scratchpad/gen_asg_delta_split.py (do not hand-edit the SQL blocks).
"""
...
ASG_TPL = """CREATE MATERIALIZED VIEW cirium.__NAME__ AS
WITH
    revs AS (SELECT id AS revision_id FROM cirium.aircraftrevision WHERE plan_type = '__PLAN__' ORDER BY id DESC LIMIT 1),
    latest_rows AS (
        SELECT DISTINCT ON (c."Registration", c."Serial Number") c.*
        FROM cirium.ciriumaircrafts c
        WHERE c.revision_id IN (SELECT revision_id FROM revs)
        ORDER BY c."Registration", c."Serial Number", c.revision_id DESC, c.id DESC
    ),
    active_latest AS (
    ...
    old_inactive AS (
        -- one row per aircraft per distinct inactive Status (newest such old revision),
        -- so a status repeated across many old revisions is NOT duplicated.
        SELECT DISTINCT ON (o."Registration", o."Serial Number", o."Status") o.id AS source_id, o.*
        FROM cirium.ciriumaircrafts o
        JOIN active_latest al2 ON al2."Registration" = o."Registration" AND al2."Serial Number" = o."Serial Number"
        WHERE o.revision_id IN (SELECT id FROM cirium.aircraftrevision WHERE plan_type = '__PLAN__') AND o.revision_id NOT IN (SELECT revision_id FROM revs)
          AND o."Status" IN ('Cancelled', 'On order', 'Retired', 'Written off')
        ORDER BY o."Registration", o."Serial Number", o."Status", o.revision_id DESC, o.id DESC
    )
SELECT al.source_id,
...
JOIN active_latest al3 ON al3."Registration" = oi."Registration" AND al3."Serial Number" = oi."Serial Number"
WITH NO DATA"""
DELTA_TPL = """CREATE MATERIALIZED VIEW cirium.__NAME__ AS
WITH
    revs AS (SELECT id AS revision_id FROM cirium.aircraftrevision WHERE plan_type = '__PLAN__' ORDER BY id DESC LIMIT 1),
    latest_rows AS (
        SELECT DISTINCT ON (c."Registration", c."Serial Number") c.*
    ...
    old_dedup AS (
        SELECT DISTINCT ON (o.revision_id, o."Registration", o."Serial Number") o.*
        FROM cirium.ciriumaircrafts o
        WHERE o.revision_id IN (SELECT id FROM cirium.aircraftrevision WHERE plan_type = '__PLAN__') AND o.revision_id NOT IN (SELECT revision_id FROM revs)
        ORDER BY o.revision_id, o."Registration", o."Serial Number", o.id DESC
    )
...
JOIN latest_rows lr2
  ON lr2."Registration" = od."Registration" AND lr2."Serial Number" = od."Serial Number"
WHERE (
...
WITH NO DATA"""
ASG_ORIG = """CREATE MATERIALIZED VIEW cirium.asg AS
WITH
    revs AS (
    ...
    old_inactive AS (
        ...
        FROM cirium.ciriumaircrafts o
        JOIN active_latest al2 ON al2."Registration" = o."Registration" AND al2."Serial Number" = o."Serial Number"
        WHERE o.revision_id NOT IN (SELECT revision_id FROM revs)
          AND o."Status" IN ('Cancelled', 'On order', 'Retired', 'Written off')
        ORDER BY o."Registration", o."Serial Number", o."Status", o.revision_id DESC, o.id DESC
    )
SELECT al.source_id,
...
JOIN active_latest al3 ON al3."Registration" = oi."Registration" AND al3."Serial Number" = oi."Serial Number"
WITH DATA"""
DELTA_ORIG = """CREATE MATERIALIZED VIEW cirium.delta AS
WITH
    revs AS (
        SELECT revision_id FROM (
            SELECT DISTINCT revision_id FROM cirium.ciriumaircrafts ORDER BY revision_id DESC LIMIT 3
...
PLANS = [("commercial", "Commercial"), ("business_helicopters", "Business&Helicopters")]


def _asg_indexes(name: str) -> None:
    op.execute(f"CREATE UNIQUE INDEX ix_{name}_source_id ON cirium.{name} (source_id)")
    op.execute(f'CREATE INDEX ix_{name}_reg_serial ON cirium.{name} ("Registration", "Serial Number")')
    op.execute(f"CREATE INDEX ix_{name}_is_active ON cirium.{name} (is_active)")
    op.execute(f"CREATE INDEX ix_{name}_revision_id ON cirium.{name} (revision_id)")
    op.execute(f'CREATE INDEX ix_{name}_airline ON cirium.{name} ("Airline")')


def _delta_indexes(name: str) -> None:
    op.execute(f"CREATE UNIQUE INDEX ix_{name}_source_id ON cirium.{name} (source_id)")
    op.execute(f"CREATE INDEX ix_{name}_is_latest ON cirium.{name} (is_latest)")
    op.execute(f"CREATE INDEX ix_{name}_revision_id ON cirium.{name} (revision_id)")
    op.execute(f'CREATE INDEX ix_{name}_reg_serial ON cirium.{name} ("Registration", "Serial Number")')
...
def upgrade() -> None:
    # create the new per-plan variants first (WITH NO DATA -> instant; a REFRESH populates them)
    for suffix, plan in PLANS:
        op.execute(ASG_TPL.replace("__NAME__", f"asg_{suffix}").replace("__PLAN__", plan))
        _asg_indexes(f"asg_{suffix}")
    op.execute("CREATE MATERIALIZED VIEW cirium.asg_full AS "
               "SELECT * FROM cirium.asg_commercial "
               "UNION ALL SELECT * FROM cirium.asg_business_helicopters WITH NO DATA")
    _asg_indexes("asg_full")

    for suffix, plan in PLANS:
        op.execute(DELTA_TPL.replace("__NAME__", f"delta_{suffix}").replace("__PLAN__", plan))
        _delta_indexes(f"delta_{suffix}")
    op.execute("CREATE MATERIALIZED VIEW cirium.delta_full AS "
               "SELECT * FROM cirium.delta_commercial "
               "UNION ALL SELECT * FROM cirium.delta_business_helicopters WITH NO DATA")
    _delta_indexes("delta_full")

    # repoint the registration sync onto asg_full, then drop the old single asg/delta
    op.execute(SYNC_TO_FULL)
    op.execute("DROP MATERIALIZED VIEW IF EXISTS cirium.asg")
    op.execute("DROP MATERIALIZED VIEW IF EXISTS cirium.delta")


def downgrade() -> None:
    op.execute(SYNC_TO_ASG)
    for name in ("asg_full", "asg_commercial", "asg_business_helicopters",
                 "delta_full", "delta_commercial", "delta_business_helicopters"):
        op.execute(f"DROP MATERIALIZED VIEW IF EXISTS cirium.{name}")
    # restore the original single asg/delta (d9e0 definitions)
    op.execute(ASG_ORIG)
