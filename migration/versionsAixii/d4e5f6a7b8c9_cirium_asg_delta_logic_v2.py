"""rebuild cirium.asg / cirium.delta materialized views with v2 business logic

Revision ID: d4e5f6a7b8c9
Revises: c3d4e5f6a7b8
Create Date: 2026-06-28

DELTA: latest revision (deduped by Registration+Serial Number, is_latest=TRUE) UNION older-revision
rows (deduped per revision by Registration+Serial Number) that REALLY differ from the latest row of
the same aircraft, IGNORING volatile metrics (Age, *_Duration_*, Previous_*, Average_*, Cumulative_*,
Reported_Hours_and_Cycles_Date). is_latest=FALSE for the older differing rows.

ASG: from the latest revision, aircraft of tracked airlines (api.airlines ILIKE match on
Operator/Sub Lessor/Owner) whose Status is NOT inactive-like ('Cancelled','On order','Retired',
'Written off') -> is_active=TRUE (deduped by Registration+Serial Number). Then for those same
aircraft, older-revision rows whose Status IS inactive-like -> is_active=FALSE (deduped per revision).
Both views now carry source_id (= ciriumaircrafts.id) as the unique key.

Forward-only: downgrade drops the materialized views (the legacy tables are gone).
"""
...
active_latest AS (
    SELECT DISTINCT ON (c."Registration", c."Serial Number")
           c.id AS source_id, a.airline_name AS airline, c.*
    ...
    ORDER BY c."Registration", c."Serial Number", c.id DESC
),
old_inactive AS (
    SELECT DISTINCT ON (o.revision_id, o."Registration", o."Serial Number")
           o.id AS source_id, o.*
    FROM cirium.ciriumaircrafts o
    CROSS JOIN latest
    JOIN active_latest al2
      ON al2."Registration" = o."Registration"
     AND al2."Serial Number" = o."Serial Number"
    WHERE o.revision_id <> latest.rid
...
WHERE (
             od."plan_type" IS DISTINCT FROM lr2."plan_type"
             OR od."Type" IS DISTINCT FROM lr2."Type"
