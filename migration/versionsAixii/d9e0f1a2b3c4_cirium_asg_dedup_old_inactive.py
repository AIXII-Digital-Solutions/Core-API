"""asg: dedup old-inactive to one row per (Registration, Serial Number, Status)

Revision ID: d9e0f1a2b3c4
Revises: c8d9e0f1a2b3
Create Date: 2026-06-28

"Current" = the 3 latest revisions (revision_id IN top-3); per aircraft the current row is the newest
in-scope one (revision_id DESC, id DESC), deduped by (Registration, Serial Number). "Old" = every
OTHER revision (revision_id NOT IN top-3), compared as history.

DELTA: current (is_latest=TRUE) UNION old rows that differ from current, ignoring plan_type + volatile
metrics (is_latest=FALSE). ASG: current rows of tracked airlines with active-like Status
(is_active=TRUE) UNION old rows of those aircraft with inactive-like Status (is_active=FALSE).
Unique key source_id (= ciriumaircrafts.id). Forward-only.
"""
