"""Aircraft insurance — schema `insurance` in the aixii database.

Everything about an insured aircraft and its losses lives here: the reference data the schedules
name, the airframe with its optional technical data, the policies and their per-aircraft records,
the claims, and the two audit trails.

The domain used to sit in schema `api` (revisions ad60f0b27298 / fa38ae0ab542) and was moved out
wholesale by `insurance_schema_move` — one schema per business domain, like every aviation source
has. Only `api.airlines` stayed behind: it is not an insurance table (the cirium asg sync writes it
and api.registration points at it), so the FKs here cross schemas on purpose.

Full rationale for every modelling decision: docs/insurance.md. Alembic reads THIS file
(db-contract); `app/Database/InsuranceModels.py` is core-api's runtime copy and has to be updated
by hand.
"""
import inspect
import sys
...
from .config import InsuranceBase as Base
# api.airlines stayed in the `api` schema (see the module docstring). A ForeignKey
# string like "api.airlines.id" is resolved inside THIS metadata, which cannot see
# another Base's table — so the cross-schema link is made with the Column object, and
# the relationships name the class itself instead of the string "Airlines".
from .ApiModels import Airlines


# --- Enumerations (native PG enums in schema `insurance`) -------------------------------------
...
class ClaimDamageType(str, PyEnum):
    ...
    reserve/paid column pairs on insurance.insurance_claims (hd_* / hw_* / hsl_*)."""
...
_STATUS_ENUM = Enum(
    InsuranceStatus, name="insurance_status", schema="insurance",
    values_callable=lambda e: [m.value for m in e],
)
_SOURCE_ENUM = Enum(
    InsuranceSource, name="insurance_source", schema="insurance",
    values_callable=lambda e: [m.value for m in e],
)
_DAMAGE_ENUM = Enum(
    ClaimDamageType, name="claim_damage_type", schema="insurance",
    values_callable=lambda e: [m.value for m in e],
)


# ==============================================================================================
# The domain, in four layers
# ==============================================================================================
# Layers:
...
class AircraftTypes(Base):
    ...
    default_engine_type_id: Mapped[Optional[int]] = mapped_column(
        BigInteger, ForeignKey("insurance.engine_types.id", ondelete="SET NULL"),
...
class Aircrafts(Base):
    ...
    aircraft_type_id: Mapped[Optional[int]] = mapped_column(
        BigInteger, ForeignKey("insurance.aircraft_types.id", ondelete="SET NULL"),
        index=True, nullable=True, default=None,
    )
    airline_id: Mapped[Optional[int]] = mapped_column(
        BigInteger, ForeignKey(Airlines.__table__.c.id, ondelete="SET NULL"),
        index=True, nullable=True, default=None,
    )
...
class AircraftSpecs(Base):
    ...
    'we have no technical data', which a nullable column on insurance.aircrafts could not distinguish
    ...
    aircraft_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("insurance.aircrafts.id", ondelete="CASCADE"), nullable=False,
    )
...
class AircraftEngines(Base):
    ...
    aircraft_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("insurance.aircrafts.id", ondelete="CASCADE"), nullable=False, index=True,
    )
...
class InsurancePolicies(Base):
    ...
    airline_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey(Airlines.__table__.c.id, ondelete="RESTRICT"), nullable=False, index=True,
    )
...
class InsuranceRecords(Base):
    ...
    insurance.insurance_record_history on every UPDATE/DELETE.
    """
    __tablename__ = "insurance_records"

    aircraft_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("insurance.aircrafts.id", ondelete="RESTRICT"), nullable=False, index=True,
    )
    policy_id: Mapped[Optional[int]] = mapped_column(
        BigInteger, ForeignKey("insurance.insurance_policies.id", ondelete="CASCADE"),
    ...
    lessee_id: Mapped[Optional[int]] = mapped_column(
        BigInteger, ForeignKey("insurance.parties.id", ondelete="SET NULL"),
        nullable=True, default=None, index=True,
    )
...
class InsuranceRecordHistory(Base):
    """Audit trail of insurance.insurance_records, written by the AFTER UPDATE/DELETE trigger
    insurance.insurance_records_audit(). Deliberately NOT a versioned-rows design: reads of the current
...
class InsuranceClaims(Base):
    """One claim = one loss event on one aircraft. Anchored on insurance.aircrafts (the stable airframe),
    ...
    same day, so there is no exclusion constraint (unlike insurance.insurance_records). `claim_reference`
    ...
    policy_id: Mapped[Optional[int]] = mapped_column(
        BigInteger, ForeignKey("insurance.insurance_policies.id", ondelete="RESTRICT"),
        nullable=True, default=None, index=True,
    )
    ...
    # --- who is handling it. Both point at insurance.parties; the column decides the role.
...
class InsuranceClaimHistory(Base):
    """Audit trail of insurance.insurance_claims, written by insurance.insurance_claims_audit().

    Unlike insurance.insurance_record_history this also captures INSERT (hence the nullable `old_row`):
