"""The insured airframes: `/fleet/types`, `/fleet/aircraft` and their engines.
...
from Database.FleetModels import Aircraft, AircraftType, AircraftEngine, MAX_ENGINES
...
class EngineIn(BaseModel):
    """One installation. Positions are the aircraft's own left-to-right as the PILOT sees it:
    two engines 1=left 2=right; three 1=left 2=centre/tail 3=right; four 1=left outboard,
    2=left inboard, 3=right inboard, 4=right outboard."""
    position: int = Field(ge=1, le=MAX_ENGINES)
    master_series: Optional[str] = Field(default=None, max_length=128, description="e.g. 'V2527-A5'.")
...
class EnginePatch(BaseModel):
    ...
    master_series: Optional[str] = Field(default=None, max_length=128)
    msn: Optional[str] = Field(default=None, max_length=64)
    installed_on: Optional[date] = None
    details: Optional[str] = None
    ...
    try:
        ...
        async with request.app.state.db_client.read_session(DB) as session:
            total = (await session.execute(
                select(func.count()).select_from(AircraftType).where(*conds))).scalar_one()
            rows = (await session.execute(stmt.limit(limit).offset(offset))).scalars().all()
        return success_response(request=request, response=response,
                                data={"items": [aircraft_type_json(t) for t in rows], "total": total})
...
@router.post(path="/types", description="Add an aircraft type. The master series is unique, "
                                        "compared trimmed and upper-cased.",
             responses=build_responses(include=_OK | {status.HTTP_201_CREATED}))
async def create_type(request: Request, response: Response, body: TypeIn,
    ...
    try:
        async with request.app.state.db_client.session(DB) as session:
            ...
            row = AircraftType(master_series=body.master_series.strip(),
                               manufacturer=body.manufacturer, template_url=body.template_url)
            session.add(row)
            await session.flush()
            data = aircraft_type_json(row)
...
@router.patch(path="/types/{type_id}", description="Change an aircraft type.",
              responses=build_responses(include=_OK))
async def update_type(request: Request, response: Response, type_id: int, body: TypePatch,
                      token: Optional[ApiToken] = Depends(authorize(SCOPE_INSURANCE_WRITE))):
    try:
        fields = body.model_dump(exclude_unset=True)
        async with request.app.state.db_client.session(DB) as session:
            await set_actor(session, token)
            row = await session.get(AircraftType, type_id)
            if row is None:
                return warning_response(request=request, response=response,
                                        msg=f"Aircraft type {type_id} not found",
...
@router.delete(path="/types/{type_id}",
               description="Remove an aircraft type. Refused with 409 while an aircraft uses it.",
               responses=build_responses(include=_OK))
async def delete_type(request: Request, response: Response, type_id: int,
                      token: Optional[ApiToken] = Depends(authorize(SCOPE_INSURANCE_WRITE))):
    try:
        async with request.app.state.db_client.session(DB) as session:
            await set_actor(session, token)
            ...
            used = (await session.execute(
                ...
                .where(Aircraft.aircraft_type_id == type_id))).scalar_one()
            if used:
                return warning_response(
                    request=request, response=response,
                    msg=f"{used} aircraft still use this type — reassign them first.",
                    status_code=status.HTTP_409_CONFLICT)
            data = aircraft_type_json(row)
            await session.delete(row)
        return success_response(request=request, response=response, data=data,
                                msg="Aircraft type deleted")
    except IntegrityError as _ex:
        code, msg = integrity_error(_ex)
        return warning_response(request=request, response=response, msg=msg, status_code=code)
    except Exception as _ex:
    ...
    try:
        async with request.app.state.db_client.session(DB) as session:
            ...
            for engine in body.engines:
                session.add(AircraftEngine(aircraft_id=row.id, **engine.model_dump()))
            await session.flush()
        ...
        async with request.app.state.db_client.session(DB) as session:
            ...
            if aircraft is None:
                return warning_response(request=request, response=response,
                                        msg=f"Aircraft {aircraft_id} not found",
                                        status_code=status.HTTP_404_NOT_FOUND)
            row = AircraftEngine(aircraft_id=aircraft_id, **body.model_dump())
            session.add(row)
            await session.flush()
